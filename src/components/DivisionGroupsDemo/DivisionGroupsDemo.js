			...
			<div className={styles.demoWrapper}>
				<div className={clsx(styles.demoArea)} style={gridStructure}>
					{range(numOfGroups).map((groupIndex) => (
						<LayoutGroup key={`${componentId}-${groupIndex}`}>
							<motion.div className={styles.group}>
								{range(numOfItemsPerGroup).map((index) => {
									return (
										<motion.div
											layoutId={`${componentId}-item-${
			...
			{includeRemainderArea === true ? (
				<div className={styles.remainderArea}>
					...
					{range(remainder).map((index) => {
						return (
							<motion.div
								key={`${componentId}-item-${numOfItems - index - 1}`}
								layoutId={`${componentId}-item-${numOfItems - index - 1}`}
								layout="position"
								className={styles.item}
							>
								<p>{numOfItems - index - 1}</p>
							</motion.div>
						);
					})}
				</div>
